...
DISJ:    LDA,Y PCH           ; loop through rest of the registers
...
SMOVE:   JSR INPUT           ; CHRIN kernal call to input a character
        STA,X INBUFF        ; store in input buffer
        INX 
        ...
        STA,X INBUFF-1      ; (replacing the CR)
...
S1:      CMP,X KEYW          ; see if input character matches
...
S2:      CPX.# $13            ; last 3 commands in table are load/save/validate
        ...
        LDA,X KADDR+1       ; push address from vector table onto stack
...
; -----------------------------------------------------------------------------
; exit monitor [X]
EXIT:    JMP.i $A002         ; jump to warm-start vector to reinitialize BASIC
...
ALTR1:  JSR GETPAR          ; get value for next register
        ...
        LDA.zp TMP0            ; store in memory, offset from SR
        STA,Y SR            ; these locations will be transferred to the
...
ALTM1:  JSR GETPAR          ; get value for next byte of memory
        ...
        LDA.zp TMP0            ; poke value into memory at start address + Y
        STA.i,Y TMP2
        INY                 ; next byte
...
DMEMGO: LDA.i,Y TMP2        ; load byte from start address + Y
...
DCHAR:  LDA.i,Y TMP2        ; load byte at start address + Y
        TAX                 ; stash in X
...
COMPAR: LDA.# 0              ; bit 7 clear signals compare
        DATA.b $2C           ; absolute BIT opcode consumes next word (LDA #$80)
...
TOKAY:  BIT SAVY            ; transfer or compare?
        ...
        LDA.zp TMP2            ; if it's a transfer, we must take steps
        CMP.zp TMP0            ;   to avoid overwriting the source bytes before 
        LDA.zp TMP2+1          ;   they have been transferred
        SBC.zp TMP0+1          ; compare source (TMP2) to destination (TMP0)
        BCS COMPAR1         ; and count up if source is before than desitnation
        LDA STORE           ; otherwise, start at end and count down...
        ADC.zp TMP0            ; add length (STORE) to desintation (TMP0)
        STA.zp TMP0            ; to calculate end of destination
        LDA STORE+1
        ADC.zp TMP0+1
        STA.zp TMP0+1
        LDX.# 1              ; change source pointer from beginning to end
TDOWN:  LDA,X STASH         ; TMP2 = source end (STASH)
        STA.zp,X TMP2
        DEX 
        BPL TDOWN
...
TCLOOP: JSR STOP            ; check for stop key
        ...
        LDA.i,Y TMP2        ; load byte from source
        BIT SAVY            ; transfer or compare?
        BPL COMPAR2         ; skip store if comparing
        STA.i,Y TMP0        ; otherwise, store in destination
COMPAR2: CMP.i,Y TMP0        ; compare to destination
...
TMVAD:  BIT UPFLG           ; counting up or down?
        ...
        INC.zp TMP0            ; increment destination low byte
        BNE TINCOK
        INC.zp TMP0+1          ; carry to high byte if necessary
        BNE TINCOK
...
HPAR:   STA,Y STAGE         ; save char in staging area
        INY 
...
HLP:    LDA.zp TMP0            ; save last read byte in staging area
        STA,Y STAGE
...
HTGO:   STY SAVY            ; save length of needle
        JSR CRLF            ; new line
HSCAN:  LDY.# 0
HLP3:   LDA.i,Y TMP2        ; get first byte in haystack
        CMP,Y STAGE         ; compare it to first byte of needle
...
LD:     LDY.# 1              ; default to reading from tape, device #1
        STY.zp FA
        STY.zp SADD            ; default to secondary address #1
        DEY
        STY.zp FNLEN           ; start with an empty filename
...
L3:     LDA,X INBUFF        ; load current char from buffer to accumulator
        BEQ LSHORT          ; no filename given, try load or verify from tape
        INX                 ; next char
        CMP.# $22            ; is it a quote?
        BEQ L8              ; if so, we've reached the end of the filename
        STA.i,Y FNADR       ; if not, save character in filename buffer
        INC.zp FNLEN           ; increment filename length
        INY 
...
L8:     STX CHRPNT          ; set character pointer to the current index
        ...
        LDA.zp TMP0            ; set device number for kernal routines
        STA.zp FA
        ...
        JSR CRLF            ; new line
        LDX.zp TMP0            ; put low byte of end address in X
        LDY.zp TMP0+1          ; put high byte of end address in Y
        LDA SAVY            ; confirm that we're doing a save
        CMP.# 'S'
        BNE LERROR          ; if not, error due to too many params
        LDA.# 0
        STA.zp SADD            ; set secondary address to 0
...
LDADDR: LDX.zp TMP2            ; load address low byte in X
        LDY.zp TMP2+1          ; load address high byte in Y
        LDA.# 0              ; 0 in A signals load
        STA.zp SADD            ; secondary addr 0 means load to addr in X and Y
...
FILL:   JSR GETDIF          ; start in TMP2, end in STASH, length in STORE
        ...
        BNE AERROR
        LDY.# 0              ; no offset
FILLP:  LDA.zp TMP0            ; load value to fill in accumulator
        STA.i,Y TMP2        ; store fill value in current address
...
ALMOR:  CMP.# $20            ; skip leading spaces
        BEQ AGET1
        STA,X MNEMW         ; put character in mnemonic buffer
        INX
...
ASQEEZ: DEX                 ; move to previous char
        ...
        LDA,X MNEMW         ; get current character
...
AADDR:  LDA.zp TMP0+1          ; check whether high byte of address is zero
...
AFIL0L: STA,X U0AA0         ; put placeholder in assembly buffer
...
AFORM1: JSR GETCHR          ; get next character
        ...
        CMP.# $20            ; skip spaces
        BEQ ASCAN
        STA,X U0AA0         ; store character in assembly buffer
        INX                 ; move to next byte in buffer
...
ATRYOP: LDX.# 0
        ...
        LDA,X MNEMR         ; check right byte of compressed mnemonic
        JSR CHEKOP
        LDA,X MNEML         ; check left byte of compressed mnemonic
        JSR CHEKOP
...
TRYMOD: ASL ACMD            ; shift a bit out of the addressing command
        ...
        LDA,X CHAR2-1       ; get second character to check
...
ABRAN:  LDY LENGTH          ; get number of bytes in operand
        ...
        LDA.zp TMP0            ; calculate the difference between the current
        SBC.zp TMP2            ;   address and the branch target (low byte)
        TAX                 ; save it in X
        LDA.zp TMP0+1          ; borrow from the high byte if necessary
        SBC.zp TMP2+1
...
OBJPUT: LDA,Y TMP0-1        ; get the operand
OBJP2:  STA.i,Y TMP2        ; store it after the opcode
        DEY
...
A1BYTE: LDA OPCODE          ; put opcode into instruction
        ...
        STA KEYD+1
        STA KEYD+6
        LDA.zp TMP2+1          ; convert high byte of next address to hex
        JSR ASCTWO
        STA KEYD+2          ; put it in the keyboard buffer
        STX KEYD+3
        LDA.zp TMP2            ; convert low byte of next address to hex
        JSR ASCTWO
...
CHEKOP: STX SAVX            ; stash X
        ...
        CMP,X U0AA0         ; check whether this opcode matches the buffer
...
DIS0AD: LDA.# $14            ; disassemble 14 bytes by default
        STA.zp TMP0            ; store length in TMP0
...
DISLIN: JSR SHOWAD          ; show the address of the instruction
        ...
        LDY.# 0              ; no offset
        LDA.i,Y TMP2        ; load operand of current instruction
...
DSBYT:  DEX                 ; decrement index
        ...
        JMP NXBYT
DSHEX:  LDA.i,Y TMP2        ; show hex for byte
        JSR WRBYTE
...
PRADR3: ASL ACMD            ; check whether addr mode uses the current char
        ...
        LDA,X CHAR1-1       ; look up the first char in the table
        JSR CHROUT          ; print first char
        LDA,X CHAR2-1       ; look up the second char in the table
...
UB64D:  LDX.zp TMP2+1          ; get high byte of current address
...
RELC2:  ADC.zp TMP2            ; add relative address to low byte
...
ERR:    LDY.# $80            ; substitute 10000000 for invalid opcodes
        LDA.# 0
GETFMT: TAX
        LDA,X MODE2         ; lookup operand format using selected nybble
...
PROPXX: TAY                 ; use index in accumulator to look up mnemonic
        LDA,Y MNEML         ;   and place a temporary copy in STORE
        STA STORE
        LDA,Y MNEMR
        STA STORE+1
...
GTNIL:  SEC                 ; set carry to indicate no parameter found
        DATA.b $24           ; BIT ZP opcode consumes next byte (CLC)
...
GOTMOD: LDY,X MODTAB        ; get base value
        LDA,X LENTAB        ; get bits per digit
...
DIGMOR: STA INDIG           ; store the digit
        ...
        LDX.# 1
DECLP1: LDA.zp,X TMP0          ; stash the previous 16-bit value for later use
        STA,X STASH
        DEX
        BPL DECLP1
NODECM: LDX NUMBIT          ; number of bits to shift
TIMES2: ASL.zp TMP0            ; shift 16-bit value by specified number of bits
        ROL.zp TMP0+1
...
NODEC2: CLC 
        ...
        ADC.zp TMP0            ; add current digit to low byte
        STA.zp TMP0            ; and store result back in low byte
        TXA                 ; A=0
        ADC.zp TMP0+1          ; add carry to high byte
        STA.zp TMP0+1          ; and store result back in high byte
...
CRLF:   LDA.# $0D            ; load CR in A
        BIT.zp $13             ; check default channel
...
GETCHR: STX SAVX
        ...
        LDA,X INBUFF        ; load next char in A
...
; -----------------------------------------------------------------------------
; copy TMP0 to TMP2
COPY12: LDA.zp TMP0            ; low byte
        STA.zp TMP2
        LDA.zp TMP0+1          ; high byte
        STA.zp TMP2+1
        RTS
...
SUB12:  SEC
        LDA.zp TMP0            ; subtract low byte
        SBC.zp TMP2
        STA.zp TMP0
        LDA.zp TMP0+1
        SBC.zp TMP2+1          ; subtract high byte
        STA.zp TMP0+1
        RTS
...
SUBA2:  STA SAVX            ; subtrahend in accumulator
        SEC
        LDA.zp TMP0            ; minuend in low byte
        SBC SAVX
        STA.zp TMP0
        LDA.zp TMP0+1          ; borrow from high byte
        SBC.# 0
        STA.zp TMP0+1
        RTS
...
BUMPAD2: CLC
        ADC.zp TMP2            ; add value in accumulator to low byte
        STA.zp TMP2
        BCC BUMPEX
        INC.zp TMP2+1          ; carry to high byte
BUMPEX: RTS 

; -----------------------------------------------------------------------------
; subtract 1 from TMP2
SUB21:  SEC
        LDA.zp TMP2            ; decrement low byte
        SBC.# 1
        STA.zp TMP2
        LDA.zp TMP2+1          ; borrow from high byte
        SBC.# 0
        STA.zp TMP2+1
        RTS
...
COPY1P: BCS CPY1PX          ; do nothing if parameter is empty
        LDA.zp TMP0            ; copy low byte
        LDY.zp TMP0+1          ; copy high byte
        STA PCL
        STY PCH
CPY1PX: RTS 
...
GETDIF: BCS GDIFX           ; exit with error if no parameter given
        ...
        LDA.zp TMP0            ; save end address in STASH
        STA STASH
        LDA.zp TMP0+1
        STA STASH+1
        ...
        DATA.b $24           ; BIT ZP opcode consumes next byte (SEC)
...
CONVRT: JSR RDPAR           ; read a parameter
        ...
        LDA.#  $24;'$'            ; output $ sigil for hex
        JSR CHROUT
        LDA.zp TMP0            ; load the 16-bit value entered
        LDX.zp TMP0+1
...
DECML1: STA,X U0AA0
        ...
        SED
DECML2: ASL.zp TMP2            ; rotate bytes out of input low byte
        ROL.zp TMP2+1          ; .. into high byte and carry bit
        LDX.# 2              ; process 3 bytes
DECDBL: LDA,X U0AA0         ; load current value of byte
        ADC,X U0AA0         ; add it to itself plus the carry bit
        STA,X U0AA0         ; store it back in the same location
...
PRINUM: PHA                 ; save accumulator
        LDA.zp TMP0            ; copy input low byte to work buffer
        STA U0AA0+2
        LDA.zp TMP0+1          ; copy input high byte to work buffer
        STA U0AA0+1
...
DSTAT:  BNE CHGDEV          ; if device address was given, use it
        ...
        DATA.b $2C           ; absolute BIT opcode consumes next word (LDX TMP0)
CHGDEV: LDX.zp TMP0            ; load device address from parameter
        ...
        LDA.# 0              ; clear status
        STA.zp SATUS
        STA.zp FNLEN           ; empty filename
        JSR GETCHR          ; get next character
        BEQ INSTAT1         ; null, display status
        DEC CHRPNT          ; back up 1 char
        CMP.#  $24 ;'$'            ; $, display directory
        BEQ DIRECT
        LDA.zp TMP0            ; command specified device to listen
        JSR LISTEN
...
INSTAT1: JSR CRLF            ; new line
        LDA.zp TMP0            ; load device address
...
DNAME:  JSR ACPTR           ; get a filename character from the device
        ...
        LDX.zp SATUS           ; check for errors or eof
...
SNDMSG: LDA,Y MSGBAS        ; Y contains offset in msg table
        PHP
...
MSGBAS:
MSG2:   DATA.b $0D               ; header for registers
...
MSG3:   DATA.b $1D
        DATA.b $3F+$80       ; syntax error: move right, display "?"
...
MSG5:   DATA.b $3A
        DATA.b $12+$80       ; ":" then RVS ON for memory ASCII dump
...
MSG7:   DATA.b $41
        DATA.b $20+$80       ; assemble next instruction: "A " + addr
...
MODE:   DATA.b $40
        ...
        DATA.b $13
        DATA.b $78
        DATA.b $A9   ; opcodes ending in 01
...
MODE2:  DATA.b $00   ; 000 000    00                  0   error
        DATA.b $21   ; 001 000    01      #$00        1   immediate
        DATA.b $81   ; 100 000    01      $00         2   zero-page
        DATA.b $82   ; 100 000    10      $0000       3   absolute
        DATA.b $00   ; 000 000    00                  4   implied
        DATA.b $00   ; 000 000    00                  5   accumulator
        DATA.b $59   ; 010 110    01      ($00,X)     6   indirect,X
        DATA.b $4D   ; 010 011    01      ($00),Y     7   indirect,Y
        DATA.b $91   ; 100 100    01      $00,X       8   zero-page,X
        DATA.b $92   ; 100 100    10      $0000,X     9   absolute,X
        DATA.b $86   ; 100 001    10      $0000,Y     A   absolute,Y
        DATA.b $4A   ; 010 010    10      ($0000)     B   indirect
        DATA.b $85   ; 100 001    01      $00,Y       C   zero-page,Y
        DATA.b $9D   ; 100 111    01      $0000*      D   relative
...
MNEML:  DATA.b $1C
        DATA.b $8A
        DATA.b $1C
        DATA.b $23   ; BRK PHP BPL CLC
        DATA.b $5D
        DATA.b $8B
        DATA.b $1B
        DATA.b $A1   ; JSR PLP BMI SEC
        DATA.b $9D
        DATA.b $8A
        DATA.b $1D
        DATA.b $23   ; RTI PHA BVC CLI
        DATA.b $9D
        DATA.b $8B
        DATA.b $1D
        DATA.b $A1   ; RTS PLA BVS SEI
        DATA.b $00
        DATA.b $29
        DATA.b $19
        DATA.b $AE   ; ??? DEY BCC TYA
        DATA.b $69
        DATA.b $A8
        DATA.b $19
        DATA.b $23   ; LDY TAY BCS CLV
        DATA.b $24
        DATA.b $53
        DATA.b $1B
        DATA.b $23   ; CPY INY BNE CLD
        DATA.b $24
        DATA.b $53
        DATA.b $19
        DATA.b $A1   ; CPX INX BEQ SED
        ; XXXYY100 opcodes
        DATA.b $00
        DATA.b $1A
        DATA.b $5B
        DATA.b $5B   ; ??? BIT JMP JMP
        DATA.b $A5
        DATA.b $69
        DATA.b $24
        DATA.b $24   ; STY LDY CPY CPX
        ; 1XXX1010 opcodes
        DATA.b $AE
        DATA.b $AE
        DATA.b $A8
        DATA.b $AD   ; TXA TXS TAX TSX
        DATA.b $29
        DATA.b $00
        DATA.b $7C
        DATA.b $00   ; DEX ??? NOP ???
        ...
        DATA.b $9C
        DATA.b $6D
        DATA.b $9C   ; ASL ROL LSR ROR
        DATA.b $A5
        DATA.b $69
        DATA.b $29
        DATA.b $53   ; STX LDX DEC INC
        ; XXXYYY01 opcodes
        DATA.b $84
        DATA.b $13
        DATA.b $34
        DATA.b $11   ; ORA AND EOR ADC
        DATA.b $A5
        DATA.b $69
        DATA.b $23
        DATA.b $A0   ; STA LDA CMP SBC
...
MNEMR:  DATA.b $D8
        DATA.b $62
        DATA.b $5A
        DATA.b $48   ; BRK PHP BPL CLC
        DATA.b $26
        DATA.b $62
        DATA.b $94
        DATA.b $88   ; JSR PLP BMI SEC
        DATA.b $54
        DATA.b $44
        DATA.b $C8
        DATA.b $54   ; RTI PHA BVC CLI
        DATA.b $68
        DATA.b $44
        DATA.b $E8
        DATA.b $94   ; RTS PLA BVS SEI
        DATA.b $00
        DATA.b $B4
        DATA.b $08
        DATA.b $84   ; ??? DEY BCC TYA
        DATA.b $74
        DATA.b $B4
        DATA.b $28
        DATA.b $6E   ; LDY TAY BCS CLV
        DATA.b $74
        DATA.b $F4
        DATA.b $CC
        DATA.b $4A   ; CPY INY BNE CLD
        DATA.b $72
        DATA.b $F2
        DATA.b $A4
        DATA.b $8A   ; CPX INX BEQ SED
        ; XXXYY100 opcodes
        DATA.b $00
        DATA.b $AA
        DATA.b $A2
        DATA.b $A2   ; ??? BIT JMP JMP
        DATA.b $74
        DATA.b $74
        DATA.b $74
        DATA.b $72   ; STY LDY CPY CPX
        ; 1XXX1010 opcodes
        DATA.b $44
        DATA.b $68
        DATA.b $B2
        DATA.b $32   ; TXA TXS TAX TSX
        DATA.b $B2
        DATA.b $00
        DATA.b $22
        DATA.b $00   ; DEX ??? NOP ???
        ...
        DATA.b $1A
        DATA.b $26
        DATA.b $26   ; ASL ROL LSR ROR
        DATA.b $72
        DATA.b $72
        DATA.b $88
        DATA.b $C8   ; STX LDX DEC INC
        ; XXXYYY01 opcodes
        DATA.b $C4
        DATA.b $CA
        DATA.b $26
        DATA.b $48   ; ORA AND EOR ADC
        DATA.b $44
        DATA.b $44
        DATA.b $A2
        DATA.b $C8   ; STA LDA CMP SBC
        DATA.b $0D
        DATA.b $20
        DATA.b $20
        DATA.b $20
...
MODTAB: DATA.b $10
        DATA.b $0A
        DATA.b $08
        DATA.b 02    ; modulo number systems
LENTAB: DATA.b $04
        DATA.b $03
        DATA.b $03
        DATA.b $01   ; bits per digit

LINKAD: DATA BREAK             ; address of brk handler
SUPAD:  DATA SUPER             ; address of entry point
